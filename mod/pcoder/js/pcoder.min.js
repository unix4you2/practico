...
function PCODER_HistorialArchivoActual()
    {
        if (PCO_PCODER_StandAlone==0)
        	...
        	PCOJS_MostrarMensaje("ERROR", "<center><b>OPCION INACTIVA!!! - FEATURE DISABLED!!!</b></center><HR>Su instalacion de {P}Coder no se encuentra integrada a <a href='https://www.practico.org'>Practico Framework</a><br>El historial de versiones y muchas otras caracteristicas solo son habilitadas cuando PCoder se integra de manera nativa con Practico Framework<br><br>Your {P}Coder setup its not embeded into <a href='https://www.practico.org'>Practico Framework</a><br>The version history and many other features are available when your PCoder is embeded into a Practico Framework setup"," modal modal-wide ");
    }
...
function PCODER_CargarUsuariosChatEstandar()
    {
        if (PCO_PCODER_StandAlone==0)
            {
        		ResultadoOperacion=PCO_ObtenerContenidoAjax(0,"index.php","PCO_Accion=PCOMOD_CargarInformexID&IDInforme=-24&PCOSESS_LoginUsuario="+PCOSESS_LoginUsuario);
