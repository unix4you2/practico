...
#PROCESA LOS ARCHIVOS
	...
	do
		...
		java -jar dev/yuic/yuicompressor-2.4.8.jar $SaltosDeLinea $SetCaracteres $NoOfuscarCodigo $MantenerComilla $NoOptimizar --type js -o "$i$PrefijoArchivo$ExtensionArchivosJS" "$i$ExtensionArchivosJS"
	done

	for i in $ListaArchivosCSS
	do
		echo "Optimizando $i$ExtensionArchivosCSS"
		java -jar dev/yuic/yuicompressor-2.4.8.jar $SaltosDeLinea $SetCaracteres $NoOfuscarCodigo $MantenerComilla $NoOptimizar --type css -o "$i$PrefijoArchivo$ExtensionArchivosCSS" "$i$ExtensionArchivosCSS"
	done
